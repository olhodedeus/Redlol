...
local function doKeyboard_media(chat_id)
    ...
    for media, default_status in pairs(config.chat_settings['media']) do
		...
		local media_texts = {
			...
			gif = _("GIFs"), -- tem
			video = _("Videos"), -- tem
			document = _("Documents"), --tem nao
			TGlink = _("telegram.me links"),
			voice = _("Vocal messages"), -- tem nao
			link = _("Links"),
			audio = _("Music"),
			sticker = _("Stickers"),
			contact = _("Contacts"), -- tem não
			game = _("Games"), -- tem não
			location = _("Locations") -- tem não
		}
